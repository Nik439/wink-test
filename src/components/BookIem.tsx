...
export default function BookItem ({bookData}: BookItemProps) {
  ...
  return (
    <li className="mb-7 xs:mb-10 max-w-200">
      <p className="block xs:hidden font-semibold text-xl mb-1.5">{bookData.volumeInfo.title}</p>
      <div className="flex">
        {bookData.volumeInfo.imageLinks?.thumbnail ?
          <img className="w-20 xs:w-26" src={bookData.volumeInfo.imageLinks.thumbnail}/>
          :
          <div className="h-36 w-26 bg-amber-100 border border-zinc-500 flex items-center">
            <p className="p-2 text-center text-sm text-zinc-600 leading-4">Immagine non disponibile</p>
          </div>
        }
        <div className="ml-3 break-words w-[calc(100%-6.5rem-0.75rem)]">
          <p className="hidden xs:block font-semibold text-xl mb-1.5">{bookData.volumeInfo.title}</p>
